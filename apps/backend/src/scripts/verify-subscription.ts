  ...
  try {
    ...
    if (!plan) {
      plan = await prisma.plan.create({
        data: { name: 'SaaS Pro', price: 199.9, status: 'ACTIVE' },
      });
    }
    ...
    if (school && school.asaasCustomerId && school.subscriptionId) {
      logger.log(
        `✅ Success! Linked to Asaas Customer: ${school.asaasCustomerId}`,
      );
      logger.log(
        `✅ Success! Linked to Subscription: ${school.subscriptionId} (Status: ${school.status})`,
      );
    } else {
