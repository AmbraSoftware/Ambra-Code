...
async function bootstrap() {
  ...
  try {
    ...
    if (school?.subscriptionId) {
      ...
      logger.warn(
        '⚠️ Subscription ID missing. Asaas Sync might have failed or is disabled in Sandbox.',
      );
    }
    ...
    if (charge.netValue === 45.0) {
      logger.log(
        `✅ Split Verified: Net Value is R$ 45.00 (R$ 50.00 - R$ 5.00 Fee).`,
      );
