...
async function bootstrap() {
  ...
  try {
    ...
    const system = await prisma.platformSystem.create({
    ...
    const plan =
      ...
      (await prisma.plan.create({
        data: { name: 'Validation Enterprise', price: 99.0 },
      }));
    ...
    // ==========================================
    logger.log(
      '🔄 [LOOP] Creating 3 Operators (Canteen Owners) with Asaas Subaccounts...',
    );
    ...
    for (let i = 0; i < 3; i++) {
      ...
      logger.log(
        `👉 [Operator ${i + 1}/3] Provisioning '${name}' (${taxId}) as INDIVIDUAL...`,
      );
      ...
      try {
        ...
        logger.error(
          `   ❌ Asaas Failed for ${name}: ${JSON.stringify(e.response?.data || e.message)}`,
        );
    ...
    // ==========================================
    logger.log(
      '🔄 [LOOP] Creating 3 Families (Guardian + Student) & Testing Finance Flow...',
    );
    ...
    for (let i = 0; i < 3; i++) {
      ...
      logger.log(
        `👉 [Family ${i + 1}/3] ${familyName} Family Setup (${cpf})...`,
      );
      ...
      const wallet = await prisma.wallet.create({
        data: { userId: guardian.id, dailySpendLimit: 100.0 },
      });

      // 3.1 TEST: RECHARGE (Real Split Check)
      logger.log(
        `   💳 Testing Recharge R$ 50.00 (Split: 5.00 Nodum / 45.00 ${operators[i].name})...`,
      );

      // Call Service
      const charge = await transactionService.prepareRecharge(
        guardian.id,
        50.0,
      );
      ...
      const webhookPayload = {
        ...
        payment: {
          ...
          value: 50.0,
          netValue: 45.0, // Should match logic
          split: [
            { walletId: operators[i].asaasWalletId, fixedValue: 45.0 },
            { walletId: process.env.ASAAS_MASTER_WALLET_ID, fixedValue: 5.0 },
          ],
        },
      };
      ...
      // Verify Stock
      const pCheck = await prisma.product.findUnique({
        where: { id: prod.id },
      });
